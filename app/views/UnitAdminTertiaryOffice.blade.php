@extends("layout-unitadmin")
@section("content")

<head>
    <title>Unit/Office Tertiaries | PNP Scorecard System</title>
</head>

<div class="label_white">
<div class="col-md-12" style="margin-top:-10px; margin-bottom:15px">
 

 @foreach($unit_offices as $unitofficename)
<h1>Unit/Office Tertiary Maintenance </h1>

<h4><i>under the {{$unitofficename->UnitOfficeName}}</i>
@foreach($secondaryoffice as $second)
<i>- {{$second->UnitOfficeSecondaryName}}</i></h4>
@endforeach
  @endforeach
 
</div>
</div>

<div class="container">
  <div class="row">
    <!--CREATE Unit Office Tertiary-->
    <div class = "col-md-4">
      <div class="panel panel-default">
          <div class="panel-heading">
            <strong>Create a Unit/Office Tertiary</strong>
          </div>
          <div class="panel-body">
            {{ Form::open(array('URL' => 'UnitAdminTertiaryOfficeControllerStore', 'method' => 'post')) }}
            
              <fieldset>
		          			<div class="row">
                  				<div class="col-sm-12 col-md-10  col-md-offset-1 ">
                  					<div class="form-group">
				                      <div class="input-group">
				                        @if ($errors->any())
				                            <ul>
				                                {{ implode('', $errors->all('<li class="error">:message</li>')) }}
				                            </ul>
				                        @endif
				                      </div>
				                    </div>
				                    
				                    <div class="form-group">
					                    <div>{{ Form::label('UnitOfficeTertiaryName', 'Tertiary Unit Office Name:') }}</div>
	            						<div style='color:black'>{{ Form::text('UnitOfficeTertiaryName', Input::get('UnitOfficeTertiaryName'), array('placeholder' => 'Unit Office Name','autocomplete' => 'off', 'class' => 'form-control')) }}</div>
	            					</div>


	            					<div class="form-group">
                        				<div>{{ Form::label('UnitOfficeSecondaryID', 'Secondary Unit Office:') }}</div>
                        		  <div style='color:black'>{{ Form::select('UnitOfficeSecondaryID', $secondary_unit_offices_id, Input::old('UnitOfficeSecondaryID'), array('class' => 'btn btn-default dropdown-toggle form-control')) }}</div>
                    				</div>

	            					<div class="form-group">
				                        <div>{{ Form::label('UnitOfficeHasQuaternary', 'Unit Office has Field:') }}</div>
				                      

				                        <div class='col-md-12' style="margin-left:-10%">
					                        <div class='col-md-6'>
					                        	<div style='color:black'>{{Form::radio('UnitOfficeHasQuaternary', 'True', true)}} True</div>
					                        </div>
					                        <div class='col-md-6'>
					                        	<div style='color:black'>{{Form::radio('UnitOfficeHasQuaternary', 'False', true)}} False</div>
					                        	</div>
				                        </div>
				                    </div>
									<br><br>
				                    <div class="form-group">
				               			{{ Form::submit('Create Tertiary Unit Office', array('class' => 'btn btn-lg btn-success btn-block')) }}
				                    </div>
                  				</div>
                  			</div>
		          		</fieldset>
            {{ Form::close() }}
          </div>
        </div>
    </div>

    <!--ALL RANKS-->
    <div class = "col-md-8">
      <div class="panel panel-primary">
            <div class="panel-heading">
                <strong>All Unit/Office Tertiaries</strong>
            </div>
           
            <div class="panel-body table-responsive">
              <table class="table" id="users-table">
                <thead>
                    <tr class="filters">       
                        <th>Tertiary Name</th>
                        <th>Secondary Unit Office</th>
                        <th>Tertiary has Quaternary</th>
                        <th>Actions</th>
                    </tr>
                </thead>
                
            </table>
            </div>
            
        </div>
    </div>
  </div>
</div>


<script type="text/javascript">

$(document).ready(function() {
    $('#users-table').DataTable({
        processing: true,
        serverSide: true,
        ajax: 'unitter',
        columns: [
      
            { data: 'UnitOfficeTertiaryName', name: 'UnitOfficeTertiaryName' },
            { data: 'UnitOfficeSecondaryName', name: 'UnitOfficeSecondaryName' },
            { data: 'UnitOfficeHasQuaternary', name: 'UnitOfficeHasQuaternary' },
            { data: 'Actions', name: 'Actions', orderable: false, searchable: false}
        ]
    });
});

</script>


<script type="text/javascript">
  $(document).ready(function() {
      $('#users-table').DataTable();
  } );
</script>

@stop